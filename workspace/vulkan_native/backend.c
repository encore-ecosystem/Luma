/* Luma-owned direct Vulkan core, shared with embedded scene renderers. */
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
...
typedef struct {
    uint32_t sType;
    void *pNext;
    uint32_t swapchainMaintenance1;
...
typedef struct {
    ...
    void *fences[2];
    void *present_fences[2];
    uint32_t frame_index;
    uint32_t requested_present_mode;
...
size_t graphene_vk_shared_offscreen_create(size_t device_handle, uint32_t width, uint32_t height);
